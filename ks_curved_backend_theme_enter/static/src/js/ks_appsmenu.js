odoo.define("ks_curved_backend_theme_enter.ks_appsmenu", function(require) {
    ...
    AppsMenu.include({
        ...
        willStart: async function() {
            ...
            this._apps = ks_fav_apps.apps;
            this._ks_frequent_apps = data;
            this.ks_fav_apps = ks_fav_apps.ids;
//            data.forEach((item, index) => {
//                var frequent_app = this._apps.filter((app) => {
//                    if (app.menuID == item) return app;
//                });
//                for (let i = 0; i < this.getParent().menu_data.children.length; i++) {
//                    if (!this.getParent().menu_data.children[i].web_icon_data && frequent_app[0].menuID === this.getParent().menu_data.children[i].id){
//                        frequent_app[0].icon_class = this.getParent().menu_data.children[i].web_icon.split(",")[0]
//                        frequent_app[0].color = this.getParent().menu_data.children[i].web_icon.split(",")[1]
//                        frequent_app[0].background_color = this.getParent().menu_data.children[i].web_icon.split(",")[2]
//
//                    }
//                  }
//                frequent_app[0].is_app_data   = frequent_app[0].web_icon_data ? true : false;
//                frequent_app[0].app_link = '#menu_id='+frequent_app[0].menuID+'&amp;action_id='+frequent_app[0].actionID;
//                frequent_app[0].app_icon = 'data:image/png;base64,'+frequent_app[0].web_icon_data
...
//            });
         ...
         * @returns {Object[]}
         */
        _getFrequentApps: function(displayedApps) {
            var ksResult = [];
            if(displayedApps){
                displayedApps.forEach((app)=>{
                    if(this._ks_frequent_apps && this._ks_frequent_apps.includes(app.id))
                        ksResult.push(app);
                });
            }
            return ksResult;
