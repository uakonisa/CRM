odoo.define("ks_curved_backend_theme_enter.ks_app_sidebar", function (require) {
  ...
  var ksAppsBar = Widget.extend({
    ...
    events: _.extend({}, Widget.prototype.events, {
      ...
      "click button.left_sidebar_arrow": "_ksToggleSideBar",
    }),
    /**
     * @override
...
//      ev.preventDefault();
    ...
    _ksToggleSideBar: function(ev) {
        var self = this;
        if($('.ks_vertical_menus').find('li.o_extra_menu_items').length){
            var ksDropdownMenu = $('.ks_vertical_menus').find('ul.dropdown-menu');
            $('ul.ks_menusections').html(ksDropdownMenu.html());

            $("ul.ks_menusections").on(
                "click",
                "a",
                function (ev) {
                  ev.preventDefault();
                  if (ev.currentTarget.getAttribute('data-menu') && ev.currentTarget.getAttribute('data-action-id')){
                    self._openMenu({
                        actionID: parseInt(ev.currentTarget.getAttribute('data-action-id')),
                        menuID: parseInt(ev.currentTarget.getAttribute('data-menu')),
                      });
                  }
                }
              );
